...
export default function SubCategoryForm({ id, type = "create" }) {
  ...
  const {
    ...
    setValue,
    setError,
  } = useForm({ resolver: zodResolver(subCategorySchema) });

  const router = useRouter();
  const handleSuccess = () => {
    reset();
    router.replace("/sub-categories?page=1&limit=10");
  };

  const createMutation = useCreateSubCategory(handleSuccess);
  const updateMutation = useUpdateSubCategory(id, handleSuccess);
  const { data, isLoading, isError, error } = useSubCategory(id);
  const {
    data: categories,
